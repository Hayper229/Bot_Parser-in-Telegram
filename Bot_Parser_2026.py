...
async def get_messages(client, channel, limit=lim):
    ...
    try:
        ...
        print(f"Ошибка при получении сообщений: {e}{colorama.Fore.RESET}")
        return []
...
async def enum_users(client, channel_link):
    try:
        ...
        print(f"Ошибка при получении пользователей: {e}{colorama.Fore.RESET}")
        return []
...
async def send_report(client, channel_link, users, messages):
    admin = 'Your account OR Your channel/Chat/Group'
    try:
