...
from telethon.tl.types import Channel, PeerUser, PeerChannel, User
import os
import colorama
import argparse
import asyncio
...
async def get_messages(channel_link, limit=10000):
        ...
        async def process_msgs(entity, label, tag_color):
            async for m in client.iter_messages(entity, limit=limit):
                if m.text:
                    ...
                    print(f"{now_p}{colorama.Fore.WHITE}[{colorama.Fore.YELLOW}Message{colorama.Fore.WHITE}] "
                          f"{colorama.Fore.GREEN}{m.text.strip()[:100]} "
                    ...
                    # HTML РЕПОРТ Cтиль
                    msg_html = (
                        f"<div style='margin-bottom: 20px; border-bottom: 1px solid #333; padding-bottom: 10px;'>"
                        ...
                        f"<span style='color: #00FF00;'>ID</span><span style='color: #FF0000;'>:</span> <span style='color: #FFFF00;'>{u_id_str}</span><br>"
                        f"<span style='color: #00FF00;'>Date</span><span style='color: #FF0000;'>:</span> <span style='color: #FFFF00;'>{m.date}</span>"
                        f"</div></div>"
                    )
...
async def func():
    ...
    if argum.c:
        ...
        for uid, info in final_users.items():
            ...
            line = (
                f"<div style='border-bottom: 1px solid #444; margin-bottom: 15px; padding-bottom: 5px;'>"
                f"<span style='color: #00FF00;'>Name</span><span style='color: #FF0000;'>:</span> <span style='color: #FFFF00;'>{info['name']}</span><br>"
    ...
    async with client:
        ...
        async def live_monitor(event):
            if event.message.text:
                ...
                now_p = f"{colorama.Fore.WHITE}[{colorama.Fore.BLUE}{time.asctime()}{colorama.Fore.WHITE}]"
                # ТЕРМИНАЛ:  LIVE стиль
                print(f"{now_p}{colorama.Fore.WHITE}[{colorama.Fore.CYAN}LIVE{colorama.Fore.WHITE}]{colorama.Fore.WHITE}[{colorama.Fore.YELLOW}UserID{colorama.Fore.WHITE}] [{colorama.Fore.MAGENTA}{u_id}{colorama.Fore.WHITE}]: "
